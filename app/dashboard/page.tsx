...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const sortedByUsage = [...storedExcuses].sort(
    ...
    const usageData = sortedByUsage.slice(0, 5).map((excuse) => ({
      name:
        excuse.situation.length > 15
          ? excuse.situation.substring(0, 15) + "..."
          : excuse.situation,
        ...
        <Card>
          ...
          <CardContent className="p-4 pt-0">
            <div className="text-2xl font-bold">
              {stats.total > 0
                ? Math.round(
                    ((stats.mostUsed?.usageCount || 0) / stats.total) * 100,
                  )
                : 0}
              %
            </div>
                ...
                {stats.recentExcuses.map((excuse) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium line-clamp-1 text-sm md:text-base">
                        {excuse.situation}
                      </span>
